...
Build the patched proxy with `docker build -f deploy/Dockerfile.proxy -t smart-exam-proxy:ready-184 deploy` and PostgreSQL with `docker build -f deploy/Dockerfile.postgres -t smart-exam-postgres:ready-184 deploy`. Build backend with `docker build -t smart-exam-backend:RELEASE backend` and frontend with `docker build --build-arg VITE_ENABLE_DEMO_LOGIN=false -t smart-exam-frontend:RELEASE frontend`. Docker uses `uv sync --frozen` and `npm ci`; Node is the same version as CI. Supply the tested image tags/digests through the deployment environment. Record a manifest with `python3 scripts/operations/release-manifest.py --help`; the script records dirty state as well as image IDs. A dirty checkout is not a reproducible release commit.
